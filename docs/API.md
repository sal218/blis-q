...
| Safe places  | ✅ `GET/POST /admin/safe-places`, `PATCH/DELETE /admin/safe-places/:id` (soft-delete; audited IDs-only). **OSM import (SP-2):** `POST /admin/safe-places/osm-search` (city+category → Overpass candidates; 502 if Overpass down) + `POST /admin/safe-places/bulk` (curated array, ≤100, transactional+audited; dedupes on `osm_id`; → `{created, skipped}`). **Images (SP-6a):** `POST /admin/safe-places/upload-url {contentType}` → `{uploadUrl, key}` (presigned PUT; content-type allowlist signed in; rate-limited); the `imageKey` is confirmed (owner + HEAD size/type ≤5 MB) on create/update — SW-1 |
