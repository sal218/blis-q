...
| P-15 | Admin ban/unban user + `GET/PATCH /admin/users` | 🔒 | 🟡 | **Sprint 4 (own slice).** Deferred from `feat/moderation-actions` (which shipped report resolve/dismiss + post `remove-content`, backend-only). Needs a new `users` ban column (e.g. `bannedAt`) + `isAuthenticated` gate integration (reject banned like `deletedAt`) + audit `moderation.user_banned`/`unbanned`. **Schema change** → deploy via the safe `db:push` (P-14 / `docs/DEPLOY.md`). `/mute` stays deferred (DPIA-gated, no model — API §12); message `remove-content` lands with chat (Sprint 5). Admin-web moderation UI wiring also pending. |
