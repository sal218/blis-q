...
- **Compliance:** messages are plaintext (moderation requirement, E2EE rejected, §5.6). **Never log message content** — on send/broadcast failure log `{ communityId, errorCode }` only. Erasure scrubs the user's messages (`content` → `[deleted]`, `senderId` → null, `deletedAt` set); export includes the user's messages.
