...
| P-10 | Mobile session-token refresh | 🔒 | 🟡 | **Implemented in `feat/mobile-token-refresh`** (mid-session path). New `POST /api/v1/auth/refresh` exchanges the stored Supabase refresh token for a rotated session, reusing login's deleted/banned gates (deleted→401+revoke; banned→403 `account_suspended`+revoke; `bannedAt` off the DTO). Client: the shared `request()` chokepoint auto-refreshes on a 401 from an authenticated endpoint (single-flight, one retry; auth paths excluded so a bad-login 401 is untouched); on refresh failure the user is signed out to login with a "session expired" notice (`AuthContext.sessionExpired` → `LoginScreen`); banned-on-refresh → suspension screen. **Deferred follow-up:** cold-start refresh (reopen app after the access token expired but the refresh token is still valid → currently signs out; unchanged) + pair with P-8 session revocation. |
