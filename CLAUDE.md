...
| P-20 | Mobile suspension UX (banned-user experience) | 🔒 | 🟡 | **Slice 1 of the Suspension & Appeals feature** (`docs/MODERATION_APPEALS.md`). Banned users can still log in and only see a generic error. **No schema change → unblocked now.** Additive `code: "account_suspended"` on the banned 403 (`server/auth.ts`) + client detection (`client/lib/api/http.ts`) → dedicated suspension screen + force-logout, surfacing export/delete (still reachable) + a v1 email "appeal" link. Codex-validated plan. UI → device test before PR. |
