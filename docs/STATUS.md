...
_Last updated: 2026-06-18 — posts merged (#19); building `fix/db-push-rls-safety` (RLS/schema-deploy hardening after the prod-drift incident)._

## Current phase

**Sprint 4 — Community posts + moderation actions (ROADMAP Sprint 4). Sprint 3 (communities + block + reports + mobile + admin) is done. Posts slice (`feat/posts`) merged (#19); current branch `fix/db-push-rls-safety` hardens schema deploys after the 2026-06-18 RLS incident. Next feature slice: moderation actions.**
...
**🎉 Sprint-1 auth scope complete** (backend auth #4/#6/#7, mobile auth UI #8, admin sign-in #9). **🎉 Sprint-2 account/GDPR complete** (profile #10, export #11, erasure #12 — P-1/P-2 closed). **🎉 Sprint-3 complete** (communities #13, block/reports #14, theme #15, communities/login mobile #16, admin slice #17). **Sprint 4 underway** (posts #19 merged).

## In progress

- **`fix/db-push-rls-safety`** — RLS/schema-deploy hardening after the **2026-06-18 prod-drift incident** (see [docs/DEPLOY.md](DEPLOY.md) incident log). Caught `drizzle-kit push --force` (the old `db:push`) proposing to **DISABLE RLS on every prod table**; aborted before applying. Found prod was **missing `password_reset_tokens`** (committed in #6, never pushed) and the **test DB had RLS disabled on all tables** — **both repaired** (prod now 17/17 RLS-enabled, 0 policies; test re-asserted + posts suite re-run 19/19). This branch: neutralizes `db:push` (no more `--force` → safe interactive orchestrator), adds `db:rls` + read-only `check:rls`, adds `docs/DEPLOY.md` (safe schema-deploy procedure + incident log), logs **P-14** (full prod↔schema parity + `check:rls` on prod before launch). **No `db:push` run in this branch.**
- **Backlog (Codex):** "Deactivate account" = a **reversible pause** (not GDPR erasure) — parked in [ROADMAP](ROADMAP.md) **Sprint 4**. Next Sprint-4 feature slice: moderation actions.
