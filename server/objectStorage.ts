...
type AssetType = "avatar" | "community" | "event" | "post";
...
const claimKey = (key: string) => `upload-claim:${key}`;

async function writeClaim(key: string, userId: string): Promise<void> {
  if (redis) {
    await redis.set(claimKey(key), userId, { ex: PENDING_CLAIM_TTL_SECONDS });
    return;
  }
  inMemoryClaims.set(key, userId);
}

async function readClaim(key: string): Promise<string | null> {
  if (redis) return redis.get<string>(claimKey(key));
  return inMemoryClaims.get(key) ?? null;
}

async function deleteClaim(key: string): Promise<void> {
  if (redis) {
    await redis.del(claimKey(key));
    return;
  }
 ...
 * UUID (never the original filename) and a pending claim is recorded so the
 * upload can later be confirmed as belonging to this user.
 */
export async function createUploadUrl(
  ...
  await writeClaim(key, userId);
  return { uploadUrl, key };
}

/**
 * Confirm an upload belongs to the claiming user, then clear the claim. Returns
 * false if no matching claim exists (expired, never created, or wrong user).
 */
export async function confirmUpload(
  key: string,
  userId: string,
): Promise<boolean> {
  const claimedBy = await readClaim(key);
  if (!claimedBy || claimedBy !== userId) return false;
  await deleteClaim(key);
  return true;
}
