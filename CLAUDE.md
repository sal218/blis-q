...
| P-24 | Community chat — mobile slice + Realtime subscription auth | 🔒 | 🟡 | **After `feat/community-chat-backend`** (backend engine done: messages API + `server/realtime.ts` private-channel publish). Build the **Chat tab** (Messages inbox + thread from `chat-screen.png` / `chat-groupchat-details-screen.png`), the `useCommunityChat` hook with the **mandatory connection lifecycle** (subscribe foreground-only; `removeChannel` on navigate-away; `removeAllChannels` on background — TRANSFER §3.9), **client-side block-filter of live broadcasts** (broadcast is per-channel, can't filter server-side), and the **Realtime subscription authorization** for private channels: client `realtime.setAuth(userJWT)` + an RLS policy on Supabase's internal `realtime.messages` + a `SECURITY DEFINER` membership-check fn (**app-table zero-policy unchanged**; fallback = opaque backend-issued channel token, inferior — no live revocation). **Mid-session subscriptions are not force-disconnected on leave/ban** (only the next subscribe re-checks) — safety follow-up. UI rich features (reactions, images, pins, presence, search, unread) layer in after the baseline thread. **Deploy:** apply the `idx_messages_community` `(communityId, createdAt, id)` index change to prod via `docs/DEPLOY.md` after the backend merge. |
