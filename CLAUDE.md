...
| P-39 | 2026-07-01 security audit — findings + disposition | 🔒 | 🟡 | **Two independent audits** (Claude multi-agent sweep + parallel ChatGPT Codex), reconciled. **0 Critical.** **FIXED:** **AUTH-1** (Realtime chat ban/erasure bypass) → `fix/ban-realtime-revoke` #53; **RL-1/2/3** (six mutations missing a rate limiter: RSVP + event update + post/message/event delete + community leave) → `fix/mutation-rate-limits`. **PENDING (tracked, mostly Low/hardening):** **IV-1** list endpoints throw on bad query params → **500 instead of 400** (switch `.parse`→`safeParse` on query schemas, or a `ZodError→400` branch); **AUDIT-PRIV-1/GDPR-1** erasure nulls `actorId` but **retains the erased user's IP** on `audit_log` rows (null/coarsen `ipAddress` in erasure + build the mandated retention-purge job — controller/DPIA call); **TXN-1** `joinCommunity`/`leaveCommunity`/`blockUser`/`unblockUser` write their audit row **outside a transaction** (wrap mutation+audit in `db.transaction`); **TXN-2** `leaveCommunity` last-admin guard is a **read-then-write TOCTOU** (unreachable until multi-admin role management — fix with that slice); **SEC-LOG-01** a few auth/firebase/notifications catch blocks log the **raw error** instead of `safeErrorCode`; **SW-1** presigned R2 PUT URLs pin **no Content-Type/Length** (fix when the R2 upload pipeline is wired); **INJ-02** search terms not **LIKE-escaped** (bound params → not injection; over-broad `%`/`_` only; admin-gated); **F-02/report-hardening** generic `POST /reports` accepts arbitrary/nonexistent `resourceId` (moderation-queue hygiene — fix with reason-categories **P-38**). |
