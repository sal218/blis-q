...
| P-16 | Admin set-`isAdmin` (`PATCH /admin/users/:id`) + admin-web user-moderation UI | 🔒 | 🟢 | Deferred from `feat/user-ban`. Admin promotion is a separate sensitive action (privilege escalation — needs its own audit + guardrails); ban `reason` storage also deferred (no privacy-safe store yet — would need a `moderation_actions` table, not audit metadata). Admin-web ban/unban UI wiring pending. |
