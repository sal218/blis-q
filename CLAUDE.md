...
* **NO PUBLIC BUCKETS**
  ...
  * **Exception (map basemap, SP-4b):** the Safe Places map's Poland **PMTiles basemap** is served from a **public** HTTP endpoint (a public R2 bucket / R2 custom domain / a Worker) with range requests, because MapLibre Native fetches it directly by URL. This is permitted: it is **public, non-sensitive OSM-derived map data** — the same map every user sees, containing **zero user data** — so a public basemap carries no privacy risk. This is the ONLY permitted public bucket, mirrors the anon-key-for-Realtime exception (§2), and is a conscious controller-signed decision — **never** put user data (avatars, uploads, venue photos) in a public bucket.
* **UUID FILENAMES ONLY**
...
A list/rail whose fetch FAILED must render a visible error + a retry, NOT the empty-state copy — otherwise a failed load looks like "there's nothing here," which on a flaky mobile connection makes a working app read as broken (the maintainer hit this on the Home rails: a cold-start fetch failed and the rails showed "no communities / no news" until a full reload). Rules for any data-backed screen/rail: expose a `retry` from the hook; on `status === "error" && list.length === 0` render an error card with a retry (the shared `client/components/RailError.tsx` for Home rails); and use the `useFocusEffect` + `requestSeq` stale-guard pattern (see `useHomeEvents.ts`) so a return-to-screen silently refetches. **Cold-start self-heal:** the INITIAL load should auto-retry ONCE after a short delay on a TRANSIENT failure (network / 5xx) before surfacing the error — a first-launch race (API server/tunnel not reachable yet, an expired-token refresh still in flight per [session.ts](client/lib/session.ts) `loadSession` + the 401 interceptor) otherwise shows the retry card for a blip that clears on its own (the exact reason navigate-away-and-back "fixed" it). Shared policy in `client/hooks/homeRailRetry.ts` (`isTransientRailError` = network|server; `railRetryDelay`); non-transient errors and silent refetches never retry. `fix/home-first-load-nav`.
...
`client/lib/session.ts` `loadSession()` treats a missing/invalid/**past** `expiresAt` as signed out (clears the store, returns null) — EXCEPT on a genuinely-expired token it first tries a cold-start refresh (P-10). A cached profile must NOT route the user into the authenticated tree with an access token the backend will reject. **P-10a (transient-failure guard):** `refreshSession()` distinguishes a **genuine rejection** (`"failed"` — no token / 401 / other 4xx → sign out) from a **transient failure** (`"offline"` — a network throw or a 5xx, where auth state is unknown). On `"offline"` the session is **kept**: `loadSession()` returns the stored session at cold-start, and the mid-session 401 interceptor (`http.ts`) does not log out — a momentary connectivity blip during a refresh must never force a re-login. Only a real rejection (`"failed"`) or a ban (`"suspended"` — 403 `account_suspended`) signs out; the next successful refresh re-checks the ban/deleted gates.
...
| P-31 | News feed — LGBT news (Poland + EU) | 📬 | 🟢 | **Pitch-deck "Support & Education / Safety, News & Initiatives" pillar** (CLAUDE pillar 3 lists "news" but it's not sprinted). LGBT news from Poland & EU. Decide **content source**: curated/admin-published (clean, controlled) vs aggregated third-party feed (copyright + a processor + moderation of external content — heavier). Recommend **admin-curated** (a `news` table + admin CRUD + a mobile feed) for v1. **Slice 1 (backend) built** in `feat/news-backend` (mockups approved: `assets/news-feed-*.png` + `news-details-*.png`): additive **`news`** table (RLS zero-policy, soft-delete, `created_by_id` set-null on erasure) + frozen `NEWS_CATEGORIES` (rights/community/health/world — 🔒 Article-9-safe editorial topics) + `NewsDTO` — **two content modes in one table**: our editorial (full `body`, no link) vs externally-sourced (`summary` + `sourceUrl` to read at origin; we never republish an outlet's full article — copyright + accuracy); `imageUrl` is null until the image slice (raw `image_key` server-internal, never serialised); `featured` = the "NA TOPIE" top story. User read `GET /api/v1/news (+?category=&search=)` + `/:id` (authenticated; featured-first, then newest; soft-deleted excluded; search over title+summary+body LIKE-escaped) + admin CRUD (`GET/POST /admin/news`, `PATCH/DELETE /:id`; requireAdmin + `adminMutationUser`-limited + transactional + audited IDs-only `news.*`, metadata null). Mirrors the resources vertical. **Schema applied to TEST+PROD via docs/DEPLOY.md (human) before CI.** (Slice 1 merged as #95.) **Slice 2 (admin newsroom web page) built** in `feat/admin-news-web` (frontend-only, consumes the slice-1 admin API): a new `admin/src/pages/NewsPage.tsx` — the "Aktualności" page (list via `DataTable` + category `Select` filter + create/edit `Drawer` + soft-delete via `useConfirm`) mirroring `ResourcesPage`. Form fields: title, summary (card excerpt), category chip-picker, source (attribution), optional body (editorial full text), optional source link, a **NA TOPIE** featured toggle — supports **both content modes** (editorial with a body / external with a summary+link). PATCH sends the full required set + `body`/`sourceUrl` as `value|null` (blank clears; mirrors `ResourcesPage`'s url handling); POST omits blank optionals. Adds `NEWS_CATEGORIES`/`NEWS_CATEGORY_META` (Polish label + chip colour)/`NewsDTO` to `admin/src/lib/types.ts`, a `newspaper` glyph to `Icon.tsx`, and an "Aktualności" nav item under **Treści** in `App.tsx`. No backend/schema/`docs/API.md`-contract change. Gate: `cd admin && npm run build` (admin excluded from root `check:types` + not built in CI) + human browser test. (Slice 2 merged as #96.) **Slice 3 (mobile feed + article-detail) built** in `feat/mobile-news` (client-only, consumes the slice-1 read API, mirrors the resources vertical): **`NewsFeedScreen`** (own header + crisis-help button, debounced server-side search, category filter chips, a **"Na topie" featured hero** in the default view, a `FlatList` of `NewsCard`s with pull-to-refresh + load-more + dim-on-replace) and **`NewsArticleScreen`** (full-bleed category-gradient hero + floating back; category chip + "NA TOPIE" badge; source · date · read-time meta [read-time only when a body exists]; **two content modes** — a non-null `body` renders the full article, else the `summary` + a "Czytaj u źródła" `Linking.openURL(sourceUrl)` CTA; an inline **"Potrzebujesz wsparcia?"** callout → the Crisis screen). Both live in the Wsparcie/**Resources stack** (like Crisis; `NewsFeed`/`NewsArticle` added to `ResourcesStackParamList`). 🔒 Since `imageUrl` is null this slice, the card thumbnail + detail hero are **category-gradient placeholders** (SVG `LinearGradient`, no `<Image>`). Front door = a new Home **"Aktualności"** section (fills the newspaper "latest activity" placeholder; a preview of the latest few + "Zobacz wszystkie" cross-navigating via `navigate("Resources", { screen: "NewsFeed"/"NewsArticle", initial: false })`). New `client/lib/api/news.ts` (`listNews`/`getArticle`), `useNews`/`useArticle`/`useHomeNews` hooks, `NewsCard`, `NEWS_CATEGORY_COLORS` + a `NewsCategoryIcon` glyph set, `strings.news.*` + `strings.home.news`/`noNews`. Client jest tests: api query/error mapping, hooks (focus-load, setCategory/setSearch refetch, load-more stop, error+retry), both screens (featured split, chip/search, card-tap-never-`openURL`, crisis nav, both content modes + sourceUrl-null guard, back, error+retry), i18n category guard, + Home News-section cross-nav. 🔒 A card tap ALWAYS opens the detail, **never** a direct external jump. Gate: full client battery (**563 client tests**) + **⚠️ iOS device test (light+dark)** before PR. (Slice 3 merged as #97.) **Slice 4 (admin news image upload) built** in `feat/news-images` — real article photos, reusing the safe-place R2 pipeline (SP-6a). Backend: a **`news`** asset type + **`R2_BUCKET_NEWS_IMAGES`** in `server/objectStorage.ts`/`env.ts` (optional in dev/test, production-required); an admin **`POST /api/admin/news/upload-url {contentType}`** presigned PUT (content-type allowlist `{jpeg,png,webp}` signed in, `adminMutationUser`-rate-limited); `createNewsSchema.imageKey` (uuid, optional) + `updateNewsSchema.imageKey` (uuid, nullable — omit=unchanged/null=remove/uuid=set); create/update **confirm** the key (owner claim + HEAD size/type ≤5 MB, delete-on-violation — **SW-1**) BEFORE the row references it (bad claim → 400); `news.image_key` column plumbed through `storage`; `toNewsDTO` now **async**, signing `imageUrl` via `getDownloadUrl("news", key)` (raw key never serialised, signed 1 h GET). Admin: an image field on `NewsPage.tsx` (request upload-url → raw `PUT` to R2 → hold key + preview; PATCH string=set/null=remove; POST string only) + a list thumbnail. Mobile: `NewsCard` thumbnail + hero and `NewsArticleScreen` banner render the real photo when `imageUrl != null`, keeping the category-gradient placeholder as fallback. Tests: +9 backend integration (upload-url 200/bad-type/403/429; create confirmed/unconfirmed; update set/null; no-image) + mobile image-vs-gradient cases. 🔒 **Human provisions the R2 news bucket (EU jurisdiction) + a CORS policy allowing the admin origin + PUT + content-type — before live upload works.** Deferred: mobile-side upload (SP-6b), old-object cleanup on replace/delete, server-side resize. (Slice 4 provisioned + admin-smoke-tested + merged as #98.) **Back-nav fix** in `feat/news-back-to-home` (client-only): a news article opened directly from the Home "Aktualności" section returns to **Home** on Back (not the Wsparcie root beneath) — `NewsArticle` gains an optional `fromHome` param set by Home's `openArticle`; Back navigates to the Home tab when `fromHome`, else `goBack()` to the feed. Props widened to `CompositeScreenProps`. Events/communities unchanged (their owning tab is a sensible Back target). **Related-articles list built** in `feat/news-related`: `GET /api/v1/news/:id/related` (`storage.listRelatedNews` — single-query **same-category-first-then-newest** ordering `(category=:cat) desc, createdAt desc, id asc`, up to 5, excludes self + soft-deleted, tops up with other categories; 404 if the article's gone; authenticated read, no rate limit) + a **"Więcej wiadomości"** section on `NewsArticleScreen` (reused `NewsCard`s; tapping one `navigation.push`es a new article so Back returns; hidden when empty/loading/error) + `useRelatedNews`/`getRelatedNews`. Additive, no schema change; +6 backend integration + client (api/hook/screen) tests. Expo-Go-testable. **Suggest-a-story ("Zaproponuj temat") — slice 1 (backend) built** in `feat/news-suggestions-backend`: additive **`news_suggestions`** table (RLS zero-policy, in `supabase/rls.sql`; indexes on `(submitter_id,created_at,id)` + `(status,created_at,id)`; **no soft-delete** — `status` is a terminal `pending → approved | declined`). 🔒 free-text `title`/`description` is user PII; `declineReason` = a **frozen coarse category** `NEWS_SUGGESTION_DECLINE_REASONS {off_topic,duplicate,unverifiable,inappropriate,other}` (behaviour/content-based, **never** protected-class) stored on the row, **never** in `audit_log`; `eraseUser` nulls **both** `submitter_id` + `reviewed_by_id` (rows retained, de-linked); **neither `NewsSuggestionDTO` nor `AdminNewsSuggestionDTO` exposes `submitterId`** (minimisation — the admin reviews content, not the person). User `POST /api/v1/news/suggestions` (new `newsSuggestionUser` 10/1h limiter; strict Zod, title req, description/sourceUrl/category optional; `201 {ok:true}` ack) + `GET /api/v1/news/suggestions/mine` (offset, caller-scoped — feeds the future Profile "Moje propozycje"); registered **before** `/news/:id`. Admin `GET /api/admin/news/suggestions` (+`?status=`) + guarded one-way `POST …/:id/approve` + `POST …/:id/decline` (frozen `reason`) — `requireAdmin` + `adminMutationUser` + transactional + audited IDs-only (`news_suggestion.submitted/approved/declined`); **approve = status transition only** (the editor authors the article via the existing `POST /admin/news`). Account export includes the user's own suggestions (Art. 20). Codex Mode A APPROVED. **Schema applied to TEST+PROD via `docs/DEPLOY.md` (human) before CI.** Deferred (slices 2–3): admin-web **"Propozycje"** queue page; mobile **suggest form** + **Profile "Moje propozycje"** list; a neutral decision notification (P-38). Remaining epic: editorial publish-date/scheduling. |
...
| P-37 | Support & Education content (pillar 3) | 📬 | 🟡 | **Pitch-deck pillar 3** (ROADMAP **Sprint 7** covers the API/screens, but the deck's specific content isn't enumerated): **LGBT rights guide**, **coming-out support**, **mental-health resources**, and **contacts to psychologists / organizations / hotlines** (≈ emergency contacts — content source still pending). Decide the content model (admin-curated `resources` + structured contacts) and the **🔒 safety bar** for hotline/crisis contacts — **accuracy is life-critical** (verified, Poland-specific, kept current). Build in Sprint 7. **Slice 1 (backend) built** (`feat/resources-backend`): additive **`resources`** table (RLS zero-policy, soft-delete, `created_by_id` set-null on erasure) + frozen `RESOURCE_CATEGORIES` (mental_health/legal_rights/community_orgs/education_careers/health_services/housing_support — 🔒 Article-9-safe content topics) + `ResourceDTO {title,category,body,url,featured,createdAt}`; user read `GET /api/v1/resources (+?category=)` + `/:id` (featured-first, soft-deleted excluded) + admin CRUD (`GET/POST /admin/resources`, `PATCH/DELETE /:id`; requireAdmin + `adminMutationUser`-limited + transactional + audited IDs-only `resource.*`). Mirrors the safe-places vertical. **Schema applied to TEST+PROD via docs/DEPLOY.md (human) before CI.** (Slice 1 merged as #77.) **Slice 2 (admin-web CRUD) built** (`feat/admin-resources-web`, frontend-only): a new `admin/src/pages/ResourcesPage.tsx` (list via `DataTable` + category `Select` filter + create/edit `Drawer` + soft-delete via `useConfirm`) consuming the slice-1 admin API — mirrors `SafePlacesPage` stripped to resources' shape (no image/coords/OSM/accessibility); **no search box** (backend list is `?category=`-only); PATCH sends the full field set so a blank Link → `url:null` clears it. `admin/src/lib/types.ts` gains a local mirror of `RESOURCE_CATEGORIES`/`RESOURCE_CATEGORY_META` (Polish label + chip colour)/`ResourceDTO`; `Icon.tsx` a `book` glyph; `App.tsx` a "Materiały" nav item under **Treści**. No backend/schema/`docs/API.md` change. Gate: `cd admin && npm run build` (admin excluded from root `check:types` + not built in CI — **infra follow-up: add `cd admin && npm run build` to CI**) + human browser test. **Slice 3 (mobile Resources feature) built** (`feat/mobile-resources`, from `assets/profile-resources.png`, consumes #77): a **dedicated 5th "Wsparcie" bottom tab** (Home · Events · **Wsparcie** · Chat · Profile — explicit product decision; the mockup's bar is illustrative). **Reworked after the on-device UX review into ONE full-bleed directory screen** (the initial hub→list split tested as confusing): `ResourcesScreen` owns its header + an **inline search box** + a **category chip row** (filters in place, active chip auto-scrolled into view) + a "Polecane" featured section (default view) + the list, with a **dim+spinner on filter/search change**; `ResourceDetailScreen` is **full-bleed** with its own floating back button (no native top bar); `ResourcesListScreen` was deleted. **Server-side search added** (the only backend change — additive `?search=` on `GET /api/v1/resources`: `resourcesListQuerySchema` + `storage.listResources` `ilike(title/body)` `likeEscape`-d, combines with `category`, blank→400; both routes; docs/API.md; 5 integration cases). Detail CTA = `Linking.openURL` "Otwórz stronę" when `url != null`. 🔒 A card tap ALWAYS opens the detail, **never** a direct external jump (safer for a vulnerable audience). New `useResources`/`useResource`, `client/lib/api/resources.ts`, `ResourceCard`, `ResourceCategoryIcon` + `BookOpen` glyphs, `RESOURCE_CATEGORY_COLORS` (mockup palette), `strings.resources.*` + `tabs.resources`. The mockup's ⚡ quick-exit is NOT built (**P-17 PAUSED**). Client jest tests + i18n category guard. Gate: full client battery + **⚠️ iOS device test** (light+dark) before PR. **Crisis contacts (safety page "Pomoc w kryzysie", `assets/safety-page-*.png`) — slice 1 (backend) built** in `feat/crisis-contacts-backend`: additive `crisis_contacts` table (RLS zero-policy, soft-delete, `created_by_id` set-null on erasure), frozen Article-9-safe `CRISIS_CONTACT_CATEGORIES` (emergency/emotional_crisis/legal/community), `CrisisContactDTO {id,name,phone,description,hours\|null,category,verified,createdAt}`; **PUBLIC** read `GET /api/v1/crisis-contacts (+?category=)` + `/:id` (safety-critical → works **signed-out**; `crisisReadIp` IP-limiter, fail-closed; emergency-first order) + admin CRUD (`GET/POST /admin/crisis-contacts`, `PATCH/DELETE /:id`; requireAdmin + `adminMutationUser` + transactional + audited IDs-only `crisis_contact.*`, metadata null). `verified` = an internal `verifiedAt` admin freshness stamp (backs the "Zweryfikowane" badge + the life-critical accuracy bar); phone validated to a life-critical format (optional leading +, digits/spaces/-/(), ≥3 digits). **112 is seeded + admin-editable, NEVER hardcoded** (content is data — [[design-for-handover]]); the public read supersedes + removes the stubbed `EmergencyContact` type + `/emergency-contacts` row. **Schema applied to TEST+PROD via docs/DEPLOY.md (human) — done on prod + test** (NOTE: `db:push` proposed DISABLE-RLS ×20 + FK drop/rename churn from accumulated prod drift, so the table was applied by hand via the Supabase SQL editor instead — see P-14). Codex Mode A APPROVED; merged as #89. **Slice 2 (admin-web CRUD) built** in `feat/admin-crisis-contacts-web` (frontend-only, consumes slice 1): a `CrisisContactsPage` (list + category `Select` filter + create/edit `Drawer` + soft-delete via `useConfirm`) mirroring `ResourcesPage`, adapted to the contact shape — name, phone (a life-critical JS validator mirroring the server format), description, optional hours (blank clears via `hours:null`), a category chip picker, and a **verified** toggle. 🔒 On edit, `verified` is re-sent ONLY when the checkbox changed, so an unrelated edit doesn't re-stamp the life-critical `verifiedAt`. Adds `CRISIS_CONTACT_CATEGORIES`/`CRISIS_CONTACT_CATEGORY_META`/`CrisisContactDTO` to `admin/src/lib/types.ts`, a `phone` glyph to `Icon.tsx`, and a "Kontakty kryzysowe" nav item under **Treści**. Gate: `cd admin && npm run build` (admin excluded from root check:types + not in CI) + human browser test. Merged as #91. **Slice 3a (mobile safety page + Wsparcie entry) built** in `feat/mobile-safety-page` (client-only, consumes the public slice-1 read): `client/screens/crisis/CrisisScreen.tsx` ("Pomoc w kryzysie", from `assets/safety-page-*.png`) — a **data-driven 112 emergency banner** (number from the emergency contact, never hardcoded; hidden if none), **client-side** category filter chips (emergency excluded — it's the banner), a "Polecane kontakty" section + "Zweryfikowane i bezpłatne" trust label, **contact cards** with a green tap-to-call (`Linking` `tel:`, digits-stripped via `telUrl`), and a "Twoje bezpieczeństwo" footer. Reached from a new **crisis-help (phone-call)** button in the **Wsparcie** header; `Crisis` is registered in the Resources stack (tab bar stays, per the light mockup). New `client/lib/api/crisisContacts.ts` + `useCrisisContacts` (fetch-all one page pageSize 100 + a dev-warn if >1 page; client-side filter), `CrisisContactCard`, `crisisCategories.ts` colors, `Lifebuoy`/`Phone`/`Lock`/`ShieldCheck`/`CrisisCategoryIcon` glyphs (Lifebuoy = the Phosphor v2 regular path), `strings.crisis.*`. Client jest tests incl. CrisisScreen safety behaviors + i18n category guard. **Verified-only publish gate added (safety + cybersec, Codex-required):** the PUBLIC read `GET /api/v1/crisis-contacts` (+ `/:id`) is now **verified-only** server-side (`isNotNull(crisisContacts.verifiedAt)` via a `verifiedOnly` flag on `listCrisisContacts`/`getCrisisContact`; the public routes pass `verifiedOnly:true`) so unverified/not-yet-vetted contacts **never leave the server** — a real publish gate (an admin adds a contact → it stays in the admin portal until they tick "Zweryfikowany"); the ADMIN list stays all. The mobile ALSO gates on `verified` (defense-in-depth). +3 backend integration tests (public excludes unverified · public `/:id` → 404 for unverified · admin list includes unverified) + client tests. **Device-test refinements:** pinned header+banner+chips (only the contacts scroll), 112 **banner-only** (no dup as a list card), horizontal-scroll chips, static safety footer. **Codex Mode A APPROVED (round 2, incl. the server-side gate); merged as #92.** **Slice 3b (global crisis-help button) built** in `feat/crisis-button-global` (client + docs only, no backend change): a shared presentational **`CrisisHeaderButton`** (the circular phone-call button) rolled out to Home/Events/Chat/Profile and refactored into the Wsparcie header so it can't drift; the Crisis screen lives in the Resources stack, so the four non-Wsparcie screens **cross-navigate** via `navigate("Resources", { screen: "Crisis" })`, and the three stack-hosted screens (Events/Chat/Profile) widen their props to `CompositeScreenProps<NativeStackScreenProps<…>, BottomTabScreenProps<AppTabsParamList>>` (no casts; Home is already a tab-screen prop). A `CrisisHeaderButton` unit test + a cross-nav assertion on each of the Home/Events/Chat/Profile/Resources suites; docs + i18n-comment wording cleanup (Lifebuoy → phone-call). Codex Mode A APPROVED (round 2 — composite-nav typing + battery commands). Gate: full client battery + **⚠️ iOS device test (light+dark, all 5 tabs)** before PR. **Deferred (tracked):** favorites/save on contacts (the mockup's bookmark); server-side category filter + pagination on mobile (until the list grows past one page); a per-card `verified` badge (minor UI — verified-only VISIBILITY is now done, above); admin manual ordering (`sortOrder`); a url/website field; **outage-resilient caching for the public safety read** (fail-closed → 429 during a Redis outage; pre-launch safety follow-up); plus the resources **"suggest a resource" → moderation** pipeline and resource images/org logos. |
...
| P-39 | 2026-07-01 security audit — findings + disposition | 🔒 | 🟡 | **Two independent audits** (Claude multi-agent sweep + parallel ChatGPT Codex), reconciled. **0 Critical.** **FIXED:** **AUTH-1** (Realtime chat ban/erasure bypass) → `fix/ban-realtime-revoke` #53; **RL-1/2/3** (six mutations missing a rate limiter: RSVP + event update + post/message/event delete + community leave) → `fix/mutation-rate-limits` #54; **IV-1** list endpoints threw on bad query params → 500 (now query `safeParse` → **400**), **TXN-1** `joinCommunity`/`leaveCommunity`/`blockUser`/`unblockUser` audit write wrapped in `db.transaction` (mutation+audit atomic; no phantom/double audit), **INJ-02** search terms **LIKE-escaped** via `likeEscape()`, **SEC-LOG-01** raw-error logs routed through `safeErrorCode` (extracted to `server/errorCode.ts`) / firebase logs name+message only → all in `fix/p39-hardening`. **PENDING (tracked, gated on other work):** **AUDIT-PRIV-1/GDPR-1** erasure nulls `actorId` but **retains the erased user's IP** on `audit_log` rows (null/coarsen `ipAddress` in erasure + build the mandated retention-purge job — controller/DPIA call); **TXN-2** `leaveCommunity` last-admin guard is a **read-then-write TOCTOU** (unreachable until multi-admin role management — fix with that slice); **SW-1 FIXED** (`feat/safe-place-images`, SP-6a — the first wired R2 upload): the presigned PUT now signs a content-type allowlist (`jpeg/png/webp`) and `confirmUpload` HEAD-checks the stored object (allowlisted image, ≤5 MB) before it's referenced, deleting it otherwise; **F-02/report-hardening** generic `POST /reports` accepts arbitrary/nonexistent `resourceId` (moderation-queue hygiene — fix with reason-categories **P-38**). |
| P-40 | Safe Places epic — sourcing, curation, map, submissions (multi-PR) | 📬 | 🟡 | **Sprint-7 "Events & Safe Places" pillar.** **Decisions locked (2026-07-03 grill):** source = **OSM browse + NGO seed** (admin browses OSM/Overpass POIs → bulk "Add to safe places", cross-checked with Polish LGBT-NGO lists: KPH, Lambda Warszawa, Miłość Nie Wyklucza, Tolerado, Trans-Fuzja); provider = **OpenStreetMap/Overpass** (free, storable, EU, no tracking — **never Google Places**: ToS forbids storing); images = **category art now** (branded gradient/illustration per category), real photos later via R2; user submissions = **fast-follow** after admin curation; in-app order = **city+category default, proximity ("near me") on location opt-in** (reuses the ephemeral `near` sort); geocoding = **Nominatim** (EU-hostable, admin-only manual/CSV entries); decline-with-reason reuses the moderation reason model (**P-38/P-21**). 🔒 **UGC + user-location are DPIA touchpoints.** 🎨 Client wants **premium UI on BOTH the admin portal AND mobile (iOS+Android)** for this feature — see [[ui-must-be-impeccable]]. **Backend shipped** (#60): read API + admin CRUD + `%3F` coord-leak fix. **Phases:** **SP-1 built** (#61) admin CRUD page (list/create/edit/soft-delete, colour-chip category picker, optional coords) — *frontend-only, consumes #60*; **SP-2 built** (`feat/admin-osm-import`) admin **"Import from OpenStreetMap"** — **MAP-FREE** (human dropped the map from SP-2; the visual map lands with SP-4): additive `safe_places.osm_id` + partial unique index (server-internal dedupe key, NOT on the DTO; applied TEST+PROD), `server/overpass.ts` (Overpass client: category→OSM-tag map, User-Agent + 15s timeout, `OverpassError`), `POST /admin/safe-places/osm-search` (city+category → candidates; **no user PII to Overpass**, raw body never logged; 502 on failure) + `POST /admin/safe-places/bulk` (curated ≤100; app-dedupe within-request + DB `onConflictDoNothing` on the partial index; transactional + per-row `safe_place.created` audit IDs-only → `{created, skipped}`) + the "Importuj z OSM" admin panel (checklist, editable category per row, bulk-add). Overpass = a new sub-processor (COMPLIANCE); Nominatim geocode-on-manual-entry still deferred; **SP-3 built** (#63) mobile **list + type-ahead search** — CategoryChip filter chips + a debounced server-side **`?search=`** (case-insensitive substring over name+city+address, LIKE-escaped) with a ✕ clear button, `SafePlaceCard`, curation disclaimer + OSM attribution footer. **Save/bookmark slice built** (`feat/safe-place-save`, mirrors `event_saves`): additive **`safe_place_saves`** table (private per-user bookmark; both FKs cascade; TEST+PROD, in rls.sql, **check:rls 19/19**), `POST`/`DELETE /safe-places/:id/save` (idempotent, visible-gated→404, rate-limited, **not audited**) + `GET /safe-places/saved`, `SafePlaceDTO.saved` (private, no count/who-saved); an interactive **Bookmark on the card** (optimistic toggle) + a 2-tab **"Zapisane" screen** (`SegmentedControl` [Wydarzenia, Bezpieczne miejsca], default Events; nav `SavedEvents`→`Saved`). 🔒 **Fixed here (found while building):** (a) the join-less safe-places EXISTS subquery rendered `${safePlaces.id}` UNqualified → collided with `safe_place_saves.id` → `saved` always false (now qualified `"safe_places"."id"`; events' joined reads were unaffected); (b) **GDPR erasure gap** — a real erasure anonymises the user row in place so the FK cascade never fires; `eraseUser` now explicitly deletes both `event_saves` + `safe_place_saves` (tested via the real erasure path); (c) the **account export** now includes `savedEvents` + `savedSafePlaces` (Art. 20). **Card redesign (`assets/event-safeplace-screen.png`) decisions locked (2026-07-04):** **photos = SP-6a (BUILT, `feat/safe-place-images`)** — admin uploads a venue photo via the web portal (first end-to-end R2 pipeline: `safeplace` asset type + bucket `R2_BUCKET_SAFE_PLACE_IMAGES`, asset-scoped upload claims, **SW-1 fixed** = content-type allowlist signed into the PUT + a HEAD size/type ≤5 MB check at confirm, delete-on-violation); additive `safe_places.image_key` (server-internal), `SafePlaceDTO.imageUrl` (signed 1 h GET, raw key never serialised, `toSafePlaceDTO` async); mobile card square thumbnail / branded placeholder; **SP-6b (deferred)** = mobile-side upload (avatars/posts) needs `expo-image-picker` + dev-client; community images, **old-object cleanup on replace/delete + an orphaned-upload sweep** (abandoned/expired claims leave R2 objects — a deferred R2-lifecycle job; `confirmUpload` deliberately never deletes on a wrong/absent claim so a non-owner can't wipe another's pending upload), and server-side resize also deferred. **detail page = BUILT** (`feat/safe-place-detail`) — tapping a card (feed or Saved tab) opens `SafePlaceDetailScreen` (full-bleed banner photo / `LinearGradient` placeholder + name + category + address + description + pinned Save + a ⋯ **Report**); Report uses a **dedicated** `POST /safe-places/:id/report` (parseId→400, checkReportRateLimit→429, reason-only schema→400, `getSafePlace` existence→404, `submitReport({resourceType:"safe_place"})`→201, IDs-only audit) — the generic `POST /reports` enum is UNCHANGED so its F-02 no-existence-check gap isn't extended to safe places; `ReportResourceType += safe_place`; new `getSafePlace`/`reportSafePlace` api + `useSafePlace` hook (mirrors `useEvent`); **coordinates never shown**. **accessibility features = BUILT** (`feat/safe-place-accessibility`) — admins mark **verified, CONFIRMED-PRESENT-ONLY** venue features (frozen `ACCESSIBILITY_FEATURES = {wheelchair_accessible, gender_neutral_restroom, free_wifi}`; 🔒 venue attributes, Article-9-safe, never identity; client extends) via multi-select toggle chips in the CRUD form; additive `safe_places.accessibility_features text[]` (first array column; TEST+PROD; check:rls 19); `SafePlaceDTO.accessibilityFeatures` defensively narrowed to the known set (no legacy value leaks); the mobile detail screen renders a ✓+label per present feature and OMITS the section when empty — **absence = unknown, NEVER "not accessible"** (a false positive would send an at-risk user to an unusable place). Deferred: **OSM pre-fill** (map OSM `wheelchair=`/`internet_access=wlan`/`toilets:*` tags → admin-confirmed suggestions), **tri-state** known-absent display, **card chips**, and the **extended flag list** (step-free entrance / accessible parking / hearing loop — client decision, trivial via the frozen tuple). **distance + "View map" = SP-4** (needs location); **opening hours = its own later slice** (store from OSM `opening_hours` or admin entry, compute open/closed in Europe/Warsaw, shown softly — "zwykle otwarte"); **reviews/stars = deferred, a client decision** — **never** pulled from Google/third-party (ToS + Article-9 leak; **never Google Places**), OSM has none, so the only option is our own moderated UGC reviews (a dedicated DPIA-gated feature) — recommend leaning on the verified/curated badge instead. **Near-me moved to SP-4** (needs `expo-location` — not installed → dev-client rebuild — + a location permission + `location_data` consent, a DPIA touchpoint); **SP-4 split into slices** (the human chose backend-first, mirroring every vertical): **SP-4a map data source = BUILT** (`feat/safe-places-markers`) — a new `GET /api/v1/safe-places/markers` returning a **trimmed** `SafePlaceMarkerDTO {id,name,category,latitude,longitude}` for **every** visible venue with **both** coords (the map plots all pins at once, so it's **unpaginated**, capped at `MAX_SAFE_PLACE_MARKERS = 1000` — beyond it, add bbox/viewport fetching), honouring the same `category`/`city`/`search` filters as the list (so the map matches the active feed filters), excluding null-coord + soft-deleted, deterministic `city,name,id` order; **backend-only, additive, NO schema change** (reuses the existing lat/lng columns), authenticated read + no rate limit (mirrors the list/`:id`), trimmed = tight leakage boundary (no description/address/city/image/saved; tap a pin → detail fetches the full DTO); registered before `/:id`; +7 integration tests incl. a `limit:2` cap/order boundary. **SP-4b slice 1 (de-risk map) = BUILT** (`feat/safe-places-map-basemap`, client-only) — validates the native map stack on-device before polishing. Adds **`@maplibre/maplibre-react-native` v11** (new-arch-only, matches `newArchEnabled:true`; native module → dev-client rebuild) + its Expo config plugin; a **dedicated full-screen `SafePlacesMapScreen`** (NOT embedded in the FlatList — avoids pan/scroll gesture conflicts; the inert map placeholder becomes a live "Zobacz na mapie" entry) rendering a self-hosted **Poland PMTiles** basemap via MapLibre Native's native **`pmtiles://`** protocol (iOS 6.10+/Android 11.8+; `EXPO_PUBLIC_BASEMAP_URL`, fully-qualified URL, public endpoint + range requests), every `/markers` venue as a native `GeoJSONSource`+circle `Layer` pin, tap→`SafePlaceDetail`, on-map OSM attribution, loading/error/empty/unavailable states. Minimal light style (`buildBasemapStyle`, Protomaps schema, **no text labels** → no third-party glyphs fetch this slice). Pure helpers (`markersToFeatureCollection`/`pressedMarkerId`) + `useSafePlaceMarkers` + `listSafePlaceMarkers` api. Tests **mock the native map**; **on-device render is the real gate** (not CI-testable). 🔒 basemap = public OSM map data → a documented **§3 exception** to no-public-buckets (controller-signed). **Human provisions before the device test:** generate the Poland PMTiles + host public on R2 + set `EXPO_PUBLIC_BASEMAP_URL`; **EAS dev-client rebuild** with the maplibre plugin (no `eas.json` yet). **SP-4b polish (deferred → slice 2 / SP-4c):** clustering, a compact embedded map + fullscreen/**list↔map sync** per `assets/safe-places-with-map.png`, a **dark basemap** + place labels (self-hosted glyphs), **per-category marker icons**, offline tiles, and **near-me** (`expo-location` + the existing `?near=` backend + `location_data` consent). Android device test deferred (iOS-first) but the native module affects both platforms. **SP-5** user **"Suggest a safe place"** → moderation queue → approve/decline-with-reason; **SP-6a real photos = BUILT** (`feat/safe-place-images`, above); **SP-6b** = the mobile-side upload pipeline (avatars/post/community images — `expo-image-picker` + dev-client). 🔒 **VETTING (non-negotiable, launch gate):** OSM import (SP-2) is a **candidate pool, NOT a vetted safe list** — OSM tags are generic venue TYPES (cafe/bar/…), never "LGBT-safe" verified. Admins (client + team, local knowledge) MUST vet every imported place and delete unfit ones; cross-check against Polish LGBT-NGO directories; **never auto-publish**. For a vulnerable audience a wrongly-"safe" place is a real danger — the human vetting IS the safety layer. In-app framing must be "LGBT-friendly places our team curates" (appropriate care), **NOT an absolute safety guarantee** — the client (controller) owns the vetting standard. 📄 **OSM attribution (ODbL):** wherever OSM-sourced data renders (the SP-3 list + SP-4 map), show "© OpenStreetMap contributors" — a licence requirement (free, but mandatory). |
