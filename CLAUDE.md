...
| P-10 | Mobile session-token refresh not implemented | 🔒 | 🟡 | **Before beta / before real usage.** `client/lib/session.ts` persists the Supabase `refreshToken` in SecureStore but nothing exchanges it — an expired access token currently means a silent 401 until re-login. Wire a refresh path (and pair with P-8 session revocation). **Observed symptom (2026-06-23 device test):** after a long session the access token expires mid-use, so screens (e.g. opening a community) fail with the generic "Spróbuj ponownie" error and Retry can't recover (it re-sends with the same dead token); logging out + back in fixes it. Beyond wiring refresh, a 401 on a once-authenticated session should route to login with a clear "session expired" message rather than the generic error/retry. |
