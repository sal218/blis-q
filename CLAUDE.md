...
| P-16 | Admin set-`isAdmin` (`PATCH /admin/users/:id`) + admin-web user-moderation UI | 🔒 | 🟢 | Admin **reports-queue** UI shipped (`feat/admin-moderation-web`, #27). Still pending: admin-web **users/ban-unban UI** (the next admin slice), admin **set-`isAdmin`** (privilege escalation — own audit + guardrails), and ban `reason` storage (no privacy-safe store yet — would need a `moderation_actions` table, not audit metadata). |
